...
public class DownloadFileDemo1 extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        b.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {showProgress(dwnload_file_path);
    ...
    void downloadFile(){
        try {URL url = new URL(dwnload_file_path);
            ...
            int bufferLength = 0;
            while ( (bufferLength = inputStream.read(buffer)) > 0 ) {fileOutput.write(buffer, 0, bufferLength);
                downloadedSize += bufferLength;runOnUiThread(new Runnable() {
                    public void run() {pb.setProgress(downloadedSize);
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                        ...
                        try {Intent intent = new Intent(DownloadFileDemo1.this, justin.class);
